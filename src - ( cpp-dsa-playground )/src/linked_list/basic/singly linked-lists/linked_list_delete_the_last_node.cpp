...
//* linked_list_delete_the_last_node.cpp
...
int main() {
    ...
    //* Set the next of the second node to nullptr, effectively removing the last node from the list
    ...
    //* Try to print the data of the third node, which has been deleted
    //* This will likely cause a runtime error, as the third node no longer exists
    cout << head->next->next->data << endl;  //* Output: Error
    cout << head->next->next->next->data << endl;  //* Output: Error
