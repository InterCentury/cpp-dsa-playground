...
int insertion_node_at_the_head_function(node **head)
    {
      //ask user for the data of the new head node 
    ...
    //This line assigns the next pointer of the newly inserted node (new_inserted_head_node) to point to the current head node (*head).
    ...
    // we update the previous pointer of the current head node to point to the newly inserted node, effectively making it the new head of the list.   
       (*head)->previous = new_inserted_head_node;
    //finally, our new node become the head node of the list   
       *head = new_inserted_head_node;

        // the time complexity of this task is:  O(1)
    }     
int main(){
  ...
  insertion_node_at_the_head_function(&head);
  //print the list after update the head node and call the display function again
  cout<< endl << "list after update the head node," << endl << endl;
  display_linked_list_function(head);
