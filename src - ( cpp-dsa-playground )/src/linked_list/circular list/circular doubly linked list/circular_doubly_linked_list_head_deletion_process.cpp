...
int list_head_deletion_function(node **head , int passed_length_of_list)
    {
                 //head deletion process----------------------------------------->
            node* traverse_to_the_previous_node = (*head)->previous; 
                 (*head) -> previous = traverse_to_the_previous_node ;
                 traverse_to_the_previous_node -> next = (*head) -> next;
              node     *pointing_to_delete = *head; // set the deletion node to th head node
...
//pass the head node to the deletion head function 
  list_head_deletion_function(&head , returned_counted_node);
