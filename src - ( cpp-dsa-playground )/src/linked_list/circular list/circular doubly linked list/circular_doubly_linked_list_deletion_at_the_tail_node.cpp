...
int deletion_node_at_the_tail_fn(node **head)
    {
         // create a node that will traverse to the tail node 
          node *traverse_to_the_second_tail_node  =( *head )-> previous -> previous;
           // crate a new temp tail node that will pointing to the current tail node of the list
              node *pointing_to_delete_tail_node = traverse_to_the_second_tail_node -> next;
               //change the connections of second tail node and the head node
                  traverse_to_the_second_tail_node -> next = *head;
                  //change the previous part of our head node by using....
                     ( *head )-> previous  = traverse_to_the_second_tail_node;

      //finally delete the tail node of the list by using .....
       delete pointing_to_delete_tail_node;                       
    }     
int main(){
 node *head = nullptr;
...
//pass the head node to the deletion head function 
  deletion_node_at_the_tail_fn(&head);
//again, print the list to check that our hrad node is deleted or not 
 cout << endl <<"list, after deleting the tail node ," << endl;
  display_linked_list_function(head);  
