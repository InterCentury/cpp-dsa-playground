...
int insertion_node_at_the_tail_function(node **head)
    {
    ...
    //                 (*head) -> previous = new_tail_node;
         ...
         // Prompt user to enter data for the new tail node
         cout << endl << "Enter the data for the new tail node :";
         int inputted_new_tail_node_data; cin >> inputted_new_tail_node_data;
         
         // Create a new node with the given data for the new tail node
         node *new_tail_node = new node(inputted_new_tail_node_data);
         
         // Create a temporary node to traverse and point to the current last node of the list
         node *temp_traverse_to_the_last_current_last_node = (*head)->previous;
         
         // Set the next pointer of the current last node to the new tail node
         temp_traverse_to_the_last_current_last_node->next = new_tail_node;
         
         // Set the previous pointer of the new tail node to the current last node
         new_tail_node->previous = temp_traverse_to_the_last_current_last_node;
         
         // Set the next pointer of the new tail node to the head node, making it circular
         new_tail_node->next = *head;
         
         // Set the previous pointer of the head node to the new tail node
         (*head)->previous = new_tail_node;
...
int main(){
  ...
  insertion_node_at_the_tail_function(&head);
  //print the list after update the tail node and call the display function again
  cout<< endl << "list after update the tail node," << endl << endl;
  display_linked_list_function(head);
